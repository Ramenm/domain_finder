...
class TestRegisteredDomains:
    ...
    def test_domain_checker_mixed_random_and_registered(self):
        ...
        print(f"    Available (incorrect): {len(registered_domains) - registered_unavailable}/{len(registered_domains)}")
        print(f"  Time elapsed: {elapsed:.2f}s")
        ...
        # At least 80% should be available (random domains are unlikely to be taken)
        assert random_available >= len(random_domains) * 0.8, (
            f"Expected at least 80% of random domains to be available, "
